...
public class Stats {
	...
	public static void setValue(OptionFile optionfile, int i, PlayerStat playerStat, String stringValue)
	{
		...
		try
		{
			...
			if(playerStat.type == 1)
				j = Integer.parseInt(stringValue) - Player.minHeight;
			if(playerStat.type == 2)
				j = Integer.parseInt(stringValue) - Player.minAge;
			...
			{
				for(int l = 0; l < modFoot.length; l++)
					if(stringValue.equals(modFoot[l]))
						j = l;
			...
			{
				for(int i1 = 0; i1 < modInjury.length; i1++)
					if(stringValue.equals(modInjury[i1]))
						j = i1;

			}
			if(playerStat.type == 7)
			{
				for(int j1 = 0; j1 < modFK.length; j1++)
					if(stringValue.equals(modFK[j1]))
						j = j1;
	...
	public static String getString(OptionFile optionfile, int i, PlayerStat playerStat)
	{
		...
		if(playerStat.type == 1)
			s = String.valueOf(value + Player.minHeight);
		// age, minimum 15 years
		if(playerStat.type == 2)
			s = String.valueOf(value + Player.minAge);
		// nationality
		if(playerStat.type == 3)
	...
	public static final PlayerStat wff =          new PlayerStat(5, 33, 3, 7, "W Foot Freq");

	public static final PlayerStat injury =       new PlayerStat(6, 33, 6, 3, "Injury T");

	public static final PlayerStat condition =    new PlayerStat(5, 33, 8, 7, "Condition");

	public static final PlayerStat wfa =          new PlayerStat(5, 33, 11, 7, "W Foot Acc");
	...
	public static final String[] modFoot =  {
		"R", "L"
	};

	public static final String[] modInjury =  {
		"C", "B", "A"
	};

	public static final String[] modFK =  {
		"A", "B", "C", "D", "E", "F", "G", "H"
	};
